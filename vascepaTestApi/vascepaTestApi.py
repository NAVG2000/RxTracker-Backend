...
@app.route('/testChart', methods=['POST'])
def testChart():
    if request.headers['Content-Type'] == 'application/json':
        ...
        vascepa = drugObj(drug, weeks, source, predictBool)
        vascepa.graph_normalizedTRx(weeks, predictBool)
        fig = vascepa.normalizedTRxChart.fig
        img = BytesIO()
        fig.savefig(img)
        img.seek(0)
